using CrudOperation.Model.Model;
using System;
using System.Collections.Generic;
using System.Linq;

namespace CrudOperation.Services.Repository
{
    public sealed class StudentRepository : IStudent
    {
        private static readonly List<Student> students = null;

        public StudentRepository()
        {
            Student student1 = new Student() { Id = 1, FirstName = "Ajit", LastName = "Verma", Age = 24, Gender = "Male", Address = "Kurla", Class = "BScIT" };
            Student student2 = new Student() { Id = 2, FirstName = "Rajesh", LastName = "Sharma", Age = 22, Gender = "Male", Address = "Ghatkopar", Class = "BScIT" };
            students?.Add(student1);
            students?.Add(student2);
        }

        public IEnumerable<Student> GetStudents()
        {
            if (students?.Count > 0)
                return students.ToArray();
            else
                return Enumerable.Empty<Student>();
        }

        public Student GetStudentById(int id)
        {
            return students?.FirstOrDefault(x => x.Id == id) ?? new Student();
        }

        public bool SaveStudent(Student student)
        {
            try
            {
                int maxId = 0;
                if (students?.Count > 0)
                    maxId = students.Max(x => x.Id);
                student.Id = maxId;
                students?.Add(student);
                return true;
            }
            catch ( Exception)
            {
                return false;
            }
        }

        public bool UpdateStudent(Student student)
        {
            try
            {
                bool result = false;
                int studentIndex = 0;
                if (students?.Count > 0)
                    studentIndex = students?.FindIndex(x => x.Id == student.Id) ?? 0;
                if (studentIndex == 0)
                    result = false;
                else
                {
                    if (students?.Count > 0)
                    {
                        students[studentIndex].FirstName = student.FirstName;
                        students[studentIndex].LastName = student.LastName;
                        students[studentIndex].Age = student.Age;
                        students[studentIndex].Gender = student.Gender;
                        students[studentIndex].Address = student.Address;
                        students[studentIndex].Class = student.Class;
                        result = true;
                    }
                }
                return result;
            }
            catch (Exception)
            {
                return false;
            }
        }

        public bool DeleteStudentById(int id)
        {
            try
            {
                bool result = false;
                int studentIndex = 0;
                if (students?.Count > 0)
                    studentIndex = students?.FindIndex(x => x.Id == id) ?? 0;
                if (studentIndex == 0)
                    result = false;
                else
                {
                    if (students?.Count > 0)
                    {
                        students.RemoveAt(studentIndex);
                        result = true;
                    }
                }
                return result;
            }
            catch (Exception)
            {
                return false;
            }
        }
    }
}
